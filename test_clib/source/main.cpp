...
int main(int argc, char **argv)
{
	...
	r = asEngine_RegisterGlobalFunction(engine, "void print()", (asFUNCTION_t)PrintSomething, asCALL_CDECL, 0); assert( r >= 0 );

	printf("Initialized engine\n");

	asIScriptFunction *func = asEngine_GetGlobalFunctionByDecl(engine, "void print()");
	asIScriptContext *ctx = asEngine_CreateContext(engine);
	asContext_Prepare(ctx, func);
